...
<body>
    ...
    (function () {
        ...
        function renderMessage(m) {
            ...
            // Check if there is a pending message we can reconcile
            if (m.sender === 'user') {
                const escapedBody = escapeHtml(m.body || '');
                const pendingEl = list.querySelector(`.chat-pending[data-body="${escapedBody}"]`)
                               || list.querySelector('.chat-pending');
                if (pendingEl) {
                    ...
                    if (attHtml) {
                        pendingEl.insertAdjacentHTML('beforeend', `<div class="bubble bubble-att">${attHtml}</div>`);
                    }
                    // Move meta to end
                    const metaEl = pendingEl.querySelector('.meta');
                    if (metaEl) {
                        pendingEl.appendChild(metaEl);
                    }
            ...
            list.insertAdjacentHTML('beforeend',
                `<div class="chat-msg ${cls}" data-id="${m.id}">${bodyHtml}${attHtml ? `<div class="bubble bubble-att">${attHtml}</div>` : ''}<div class="meta">${time}</div></div>`);
            list.scrollTop = list.scrollHeight;
        }
